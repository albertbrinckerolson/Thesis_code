...
function main_o_all_experiment_call(experiment:: Experiment, optimizer:: DataType, demand_file::String, instance_file::String,static::Bool, model = nothing, model_dyn = nothing, threshold = 0.5, choice_model = no_choice, partial_pricing = false)
    ...
    #process_order = [i for i in 1:length(list_of_sub_graphs)]
    ...
    one_more = false
    while (length(path_list)>0) || one_more
        if one_more 
            one_more = false
        end
        ...
        println(length(path_list))
        if  (((number_of_iterations > 8) & !isnothing(model_dyn)) & partial_pricing) ||  (((number_of_iterations > 3) & !isnothing(model_dyn)) & !partial_pricing) # ||((length(path_list) < 500) & !isnothing(model_dyn))
        ...
        if  (((number_of_iterations > 8) & !isnothing(model)) & partial_pricing & isnothing(model_dyn)) ||  (((number_of_iterations > 3) & !isnothing(model)) & !partial_pricing & isnothing(model_dyn)) # ||((length(path_list) < 500) & !isnothing(model_dyn))
            println("FALLBACK at iteration ", number_of_iterations)
            fallback_it = number_of_iterations + 1
            objval_at_fallback = objective_value(master)
            for sub in list_of_sub_graphs
        ...
        if partial_pricing & (length(path_list) < 1) & !one_more
...
function experiment_loop(trials :: Int, optimizer_name:: String, demand_file::String, instance_file::String,static::Bool, model_name::String , model_dyn_name::String, threshold = 0.5, choice_model = no_choice, partial_pricing = false)
    ...
    model_dyn = nothing
    model = nothing
    if contains(model_name, "RF") || contains(model_dyn_name, "RF")
        ...
        elseif (model_dyn_name == "nothing") & (model_name != "nothing")
            ...
            model_dyn = machine(model_dyn_name)
...
function main2()
    #model_folder = "RF_models_inpath_2/"
    #model_folder = "Nye_Modeller_inpath/"
    ...
    thresholds = [0.5]#0.35 ,0.4, 0.45, 0.5, 0.55]
    ...
    for instance in instances
        ...
        experiment00 = experiment_loop(30, "Gurobi", demand, instance, false,"nothing", "nothing", 0.0, no_choice, true)
        experiment00.format_2_row("partial-test.csv", true)
        experiment00 = experiment_loop(30, "Gurobi", demand, instance, false,"nothing", "nothing", 0.0, no_choice, false)
        experiment00.format_2_row("base-test.csv", true)

        for choice_model in choice_models
            for threshold in thresholds
                #experiment05 = experiment_loop(10, "Gurobi", demand, instance, false,model_folder*stat_dict[instance], "nothing", threshold, choice_model, false)
                #experiment05.format_2_row("experiment_static.csv", true)
                #experiment05 = experiment_loop(3, "Gurobi", demand, instance, false,model_folder*stat_dict[instance], "nothing", threshold, choice_model, true)
                #experiment05.format_2_row("experiment_static_RF.csv", true)
            end
        end
    end
end
