...
function main2()
    model_folder = "Static_Static_modeller/"
    ...
    for filename in readdir(model_folder)
        for instance in instances
            if contains(filename,instance)
                ...
                if contains(filename,"inopt")
                    inopt_dict[instance] = filename
                else
                    inpath_dict[instance] = filename
                end
            end
        end
    end
    ...
    for instance in instances
        ...
        for choice_model in choice_models
            for threshold in thresholds
                #experiment05 = experiment_loop(3, "Gurobi", demand, instance, false,model_folder*inopt_dict[instance], model_folder*inpath_dict[instance], threshold, choice_model, false)
                #experiment05.format_2_row("experiment_static_static_EA3.csv", true)
                experiment05 = experiment_loop(3, "Gurobi", demand, instance, false,model_folder*inopt_dict[instance], model_folder*inpath_dict[instance], threshold, choice_model, true)
                experiment05.format_2_row("experiment_static_static_partial.csv", true)
            end
        end
    end
end
