...
function main_o_all_experiment_call(experiment:: Experiment, optimizer:: DataType, demand_file::String, instance_file::String,static::Bool, model = nothing, model_dyn = nothing, threshold = 0.5, choice_model = no_choice, partial_pricing = false)
    ...
    while (length(path_list)>0) || !fallback || one_more
        ...
        if  (((number_of_iterations > 10) & !isnothing(model)) & partial_pricing & isnothing(model_dyn)) ||  (((number_of_iterations > 3) & !isnothing(model)) & !partial_pricing & isnothing(model_dyn)) # ||((length(path_list) < 500) & !isnothing(model_dyn))
            println("FALLBACK at iteration ", number_of_iterations)
            fallback_it = number_of_iterations + 1
            objval_at_fallback = objective_value(master)
            for sub in list_of_sub_graphs
...
function main2()
    ...
    thresholds = [0.35,0.4,0.5, 0.45, 0.55]
    choice_models = [no_choice, test_choice]
    ...
    for instance in instances
        ...
        #experiment00 = experiment_loop(3, "Gurobi", demand, instance, false,"nothing", "nothing", 0.0, no_choice, false)
        #experiment00.format_2_row("test_base_n_rf.csv", true)

        #experiment00 = experiment_loop(3, "Gurobi", demand, instance, false,"nothing", "nothing", 0.0, no_choice, true)
        #experiment00.format_2_row("test_partial_n_rf.csv", true)
        for choice_model in choice_models
            for threshold in thresholds
                #experiment05 = experiment_loop(3, "Gurobi", demand, instance, false,model_folder*stat_dict[instance], "nothing", threshold, choice_model, false)
                #experiment05.format_2_row("experiment_static_thresholds_EA3_choice.csv", true)
