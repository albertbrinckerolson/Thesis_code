...
mutable struct Experiment
    ...
    vector_of_it_times :: Vector{Vector{Float64}}
end
    ...
    if s ==:format_2_row
        function(filename::String,append = true)
            ...
            new_row = DataFrame(:instance_name => this.instance_name,
                                ...
                                :vector_of_it_times => vect_field_dict[:vector_of_it_times])
...
function main_o_all_experiment_call(traveltime,demand,experiment:: Experiment, optimizer:: DataType, demand_file::String, instance_file::String,static::Bool, model = nothing, model_dyn = nothing, threshold = 0.5, choice_model = no_choice, partial_pricing = false)
    num_subs = 30
    ...
    vector_of_it_times = Vector{Float64}()
 
    env = create_env(10000.0,2.0,1.0, demand_file, instance_file, traveltime, demand)
    ...
    if !partial_pricing
        ...
        time_sub = time()
        if isnothing(model)
            list_of_path_lists, fallbacks, label_algo_calls, num_sub  = solve_sub_problems_oall_partial_pricing(list_of_sub_graphs, G,  threshold, choice_model, 1, num_subs, process_order, 3000, 16, model, model_dyn)
    ...
    push!(vector_of_it_times, time()-total_time)
    ...
    while (length(path_list)>0) || !fallback || one_more
        ...
        if !partial_pricing    
            ...
            time_sub = time()
            if isnothing(model_dyn)
                list_of_path_lists, fallbacks, label_algo_calls, num_sub  = solve_sub_problems_oall_partial_pricing(list_of_sub_graphs, G,  threshold, choice_model, num_sub, num_subs, process_order, 3000, 16, model, model_dyn)
            else
                list_of_path_lists, fallbacks, label_algo_calls, num_sub  = solve_sub_problems_oall_partial_pricing(list_of_sub_graphs, G,  threshold, choice_model, num_sub, num_subs, process_order, 3000, 16, model, model_dyn)
            end
            elapsed_time = time() - time_sub
        ...
        println(length(path_list))
        if  (((number_of_iterations > 10) & !isnothing(model_dyn)) & partial_pricing) ||  (((number_of_iterations > 3) & !isnothing(model_dyn)) & !partial_pricing) # ||((length(path_list) < 500) & !isnothing(model_dyn))
        ...
        if partial_pricing & (length(path_list) < 1) & !one_more & isnothing(model_dyn) & isnothing(model)
            subs_to_check = length(list_of_sub_graphs) - num_subs
            checked = num_subs
            while checked < subs_to_check
                list_of_path_lists, fallbacks, label_algo_calls, num_sub  = solve_sub_problems_oall_partial_pricing(list_of_sub_graphs, G,  threshold, choice_model, num_sub, num_subs, process_order, 3000, 16, model, model_dyn)
                path_list = syncronize_all(list_of_path_lists)
                if length(path_list) > 0
                    ...
                    if subs_to_check - checked < num_subs
                        num_subs = subs_to_check - checked
                    end
                    println(checked)
                end
            end
        ...
        #if fallback &&  ((length(path_list) < 100) && (isnothing(model_dyn)))
    ...
    push!(experiment.vector_of_it_times, vector_of_it_times)
    return 
end

function experiment_loop(traveltime,demand,trials :: Int, optimizer_name:: String, demand_file::String, instance_file::String,static::Bool, model_name::String , model_dyn_name::String, threshold = 0.5, choice_model = no_choice, partial_pricing = false)
    experiment = Experiment(instance_file, String(Symbol(choice_model)), optimizer_name,threshold, model_name, model_dyn_name,Vector{Float64}(), Vector{Float64}(), Vector{Float64}(), 
                                        Vector{Float64}(), Vector{Float64}(),traveltime,demand, Vector{Int}(),  Vector{Int}(),  Vector{Int}(),  Vector{Int}(), 
                                        Vector{Int}(),Vector{Float64}(),
                                        Vector{Vector{Float64}}(), Vector{Vector{Float64}}(), Vector{Vector{Float64}}(),
                                        Vector{Vector{Int}}(), Vector{Vector{Int}}(), Vector{Vector{Int}}(),Vector{Vector{Float64}}(), Vector{Vector{Float64}}())
    ...
    if optimizer_name == "Gurobi"
        for i = 1:trials
            main_o_all_experiment_call(traveltime,demand,experiment, Gurobi.Optimizer, demand_file, instance_file, false, model, model_dyn, threshold, choice_model, partial_pricing)
        end
    else
        for i = 1:trials

            main_o_all_experiment_call(traveltime,demand,experiment, GLPK.Optimizer, demand_file, instance_file, false, model, model_dyn, threshold, choice_model, partial_pricing)
...
function main2()
    ...
    for instance in instances
        ...
        #experiment00.format_2_row("test_base_n.csv", true)

        #experiment00 = experiment_loop(2, "Gurobi", demand, instance, false,"nothing", "nothing", 0.0, no_choice, true)
        #experiment00.format_2_row("test_partial_n.csv", true)
        tts = [0.0,15.0,25.0,50.0,75.0,100.0,125.0]
        dems = [0.5,0.75,1.0,1.25,1.5]
        for dem in dems
            for choice_model in choice_models
                for threshold in thresholds
                    experiment05 = experiment_loop(100.0,dem,10, "Gurobi", demand, instance, false,model_folder*stat_dict[instance], model_folder*dyn_dict[instance], threshold, choice_model, false)
                    experiment05.format_2_row("Travel_demand_WL_SoDp.csv", true)
                    #experiment05 = experiment_loop(10, "Gurobi", demand, instance, false,model_folder*stat_dict[instance], model_folder*dyn_dict[instance], threshold, choice_model, false)
                    #experiment05.format_2_row("test_simon.csv", true)
                end
            end
        end
        for tt in tts
            for choice_model in choice_models
                for threshold in thresholds
                    experiment05 = experiment_loop(100.0,dem,10, "Gurobi", demand, instance, false,model_folder*stat_dict[instance], model_folder*dyn_dict[instance], threshold, choice_model, false,tt,1.0)
                    experiment05.format_2_row("Travel_demand_WL_SoDp.csv", true)
                    #experiment05 = experiment_loop(10, "Gurobi", demand, instance, false,model_folder*stat_dict[instance], model_folder*dyn_dict[instance], threshold, choice_model, false)
                    #experiment05.format_2_row("test_simon.csv", true)
                end
