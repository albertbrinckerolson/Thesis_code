...
function main_o_all_experiment_call(experiment:: Experiment, optimizer:: DataType, demand_file::String, instance_file::String,static::Bool, model = nothing, model_dyn = nothing, threshold = 0.5, choice_model = no_choice, partial_pricing = false)
    ...
    true_preds = JLD2.load("ytrue_heltnyny.jld2")["data"]
    if instance_file == "WorldLarge_base.txt"
        preds = true_preds["Data_inpath_medsidstesub_2/"*"Instance-WL.txt_static.csv"]
    else
        preds = true_preds["Data_inpath_medsidstesub_2/"*instance_file*".csv"]
    end
    ind = 1
    println(length(list_of_sub_graphs))
    for sub in list_of_sub_graphs
        for edge_id in sub.data_frame.edge_id
            sub.edges_ordered[edge_id].prob = preds[ind]
            ind+=1
        end
    end
    println(ind)
...
function main2()
    ...
    for instance in instances
        ...
        experiment00.format_2_row("test_1.csv", true)
