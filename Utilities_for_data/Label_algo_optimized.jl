...
function backtrack!(label::Label, label_dict::Dict{Int,Label}, Gs::GraphSub, G::Graph)
    ...
    while current_label.current_node != Gs.source_base_id
        if Gs.edges_list[(current_label.current_node, previous_node)].type ∉ ["sink_edge", "load_edge", "forfeit_edge"]
            Gs.data_frame.in_path[Gs.edges_list[(current_label.current_node, previous_node)].obs_id] = 1
        end
...
function extend!(label_queue::PriorityQueue{Label,Float64}, edge::EdgeSub,num_labels, label::Label, Gs::GraphSub, dead_end::Int)
    ...
    """
        if (label.accumulated_time + edge.time <= Gs.max_time) & (edge.ij[2] ∉ label.visited) & (label.dead_ends_in_row + dead_end <2) & (label.num_dead_ends < 4)
                if dead_end == 0
                    num_dead_ends_in_row = 0
                else
                    num_dead_ends_in_row = dead_end + label.dead_ends_in_row
                end
                ...
                new_label = Label(num_labels,
                                ...
                                label.id, 
                                label.num_dead_ends + dead_end,
                                num_dead_ends_in_row)
                enqueue!(label_queue,new_label,new_label.accumulated_time)
    end
end

function process_edge(edge::EdgeSub, G::Graph, Gs::GraphSub, path_list::Vector{Path}, label::Label, label_queue::PriorityQueue{Label,Float64}, label_dict::Dict{Int64,Label}, num_labels::Int, max_dual::Float64, dead_end::Int)
    ...
    elseif (label.accumulated_cost - max_dual + 0.000001 >= 0)
        ...
        new_labels_num += 1
        extend!(label_queue, edge, num_labels+new_labels_num, label, Gs, dead_end)
    end
...
function semi_stoc_labeling_algorithm_opt!(G::Graph, path_list::Vector{Path}, Gs::GraphSub, threshold::Float64, choice_function::Function, max_dual::Float64)
    ...
    label_queue = PriorityQueue{Label,Float64}(Label(1,Set{Int}(), Gs.source_base_id, 0, 0, 0, 0, 0, 0) => 0)
    ...
    while isempty(label_queue) == false
        ...
        if dominated
            ...
            for edge in Gs.node_dict[label.current_node].outgoing_edges
                if edge.type in ["forfeit_edge", "load_edge", "sink_edge"]
                    #no_probable = false
                    if string(choice_function) == "test_choice"
                        no_probable = false
                    end
                elseif edge.prob < threshold  
                    continue
                else
                    no_probable = false
                end
                # check if current edge is connecting path to sink
                labels, paths = process_edge(edge, G, Gs, path_list, label, label_queue, label_dict,num_labels, max_dual, 0)
                num_labels += labels
                num_paths += paths

            end
            if no_probable & (threshold > 0.0)
                new_edge_list = choice_function(Gs.node_dict[label.current_node].outgoing_edges, threshold)

                for edge in new_edge_list
                    labels, paths = process_edge(edge, G, Gs, path_list, label, label_queue, label_dict,num_labels, max_dual, 1)
